...
# Modified by Indranil Sinharoy, Luan Nguyen (2020)
...
if __name__ == "__main__":
    ...
    print('\nBrooks MFC using SmartDDE server\n')
    print('\nPress Ctrl+{0} to exit'.format('Break' if os.name == 'nt' else 'C'))
    
    # setup the rs485 comm port:
    server_name="SmartDDE"
    tag_number="28478010"
    com="COM5"
    communication_type="!RTS" #(!)RTS or (!)DTR for RS485 orthewise RS232
    baud="19200"
    parity="ODD"
    topic_name=tag_number+":" + com  + "," + communication_type + "," + baud + "," + parity
    server_name, topic_name

    
    # Create connection to SmartDDE
    dde = DDEClient(server_name, topic_name)
    
    try:
        dde.advise("[337]") # setpoint percent
        dde.advise("[338]") # flow percent
        WinMSGLoop() # Run the main message loop to receive advices
